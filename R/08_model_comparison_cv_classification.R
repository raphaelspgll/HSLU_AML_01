...
df_all <- heapo |>
  select(
    high_consumption,
    heating_degree_days, temp_avg, sunshine_hours, humidity_avg,
    living_area, building_type, heatpump_type, has_floor_heating,
...
svm_cols <- c(
  ...
  "heating_degree_days", "sunshine_hours", "humidity_avg",
...
cv_class <- lapply(seq_along(folds), function(i) {
  ...
  svm_prob <- kernlab::predict(svm_fit, newdata = svm_test, type = "probabilities")[, "high"]
  svm_pred <- kernlab::predict(svm_fit, newdata = svm_test)

  actual_int <- df_all[test_idx, "high_consumption_int"]
  actual_fac <- df_all[test_idx, "high_consumption_fac"]
