...
cat(
  "Annual consumption calculated:",
  nrow(annual_consumption),
  "household-year combinations\n"
)
...
weather_annual <- map_dfr(weather_files, function(f) {
  ...
  df %>%
    ...
    summarise(
      ...
      temp_range = mean(
        Temperature_max_daily - Temperature_min_daily,
        na.rm = TRUE
      ),
...
df <- meta %>%
  ...
  left_join(
    households %>% mutate(Household_ID = as.character(Household_ID)),
    by = "Household_ID"
  )
...
heapo_final <- df %>%
  transmute(
    ...
    kWh_per_m2 = ifelse(
      !is.na(as.numeric(Survey_Building_LivingArea)) &
        as.numeric(Survey_Building_LivingArea) > 0,
      annual_kWh_total / as.numeric(Survey_Building_LivingArea),
      NA
    ),
    ...
    high_consumption = as.integer(
      annual_kWh_total > median(annual_kWh_total, na.rm = TRUE)
    )
  )
...
cat(
  "Predictors:",
  ncol(heapo_clean) - 3,
  "(excluding ID, response, high_consumption)\n"
)
...
numeric_vars <- names(heapo_clean)[
  sapply(heapo_clean, is.numeric) &
    !names(heapo_clean) %in%
      c(
        "Household_ID",
        "annual_kWh_total",
        "high_consumption",
...
for (col in factor_vars) {
  cat(sprintf(
    "- Categorical: %s (%d levels)\n",
    col,
    nlevels(heapo_clean[[col]])
  ))
}

cat("- Count: n_residents\n")
cat(
  "- Binary: has_floor_heating, has_radiator,",
